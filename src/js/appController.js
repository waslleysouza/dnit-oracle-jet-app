...
function (oj, ko, $, data, mapping, PushClient, ConnectionDrawer) {
  ...
  router.configure({
    ...
    'createIncident': { label: 'Criar um Recurso' }
  });
  ...
  function AppControllerViewModel() {
    ...
    // Navigate to customer by id
    self.goToDriver = function(id) {
      self.router.go('drivers/driverDetails/' + id);
    };
