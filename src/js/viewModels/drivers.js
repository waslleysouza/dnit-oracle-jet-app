...
define(['ojs/ojcore', 'jquery', 'appController', 'ojs/ojinputtext'], function (oj, $, app) {
  function driversViewModel() {
    ...
    self.handleActivated = function(params) {
      ...
      self.router = parentRouter.createChildRouter('drivers').configure({
        'driversList': { label: 'Drivers List', isDefault: true },
        'driverDetails': { label: 'Driver Details' },
        'driverCreate': { label: 'Add Driver', canEnter: function() {
            if(self.router.stateId() === 'driverDetails') {
              window.history.back();
              return false;
            } else {
              return true;
